    ...
    public async send(trx: string, payload: any) {
        return new Promise<ITransaction>(async (resolve, reject) => {
            ...
            if (authStore.loginAuth === 'keychain' && window.steem_keychain) {
                window.steem_keychain.requestBroadcast(authStore.username, transaction.operations, 'Active', function (response: any) {
                    console.log(response);
                    resolve(response)
                });
            }
            else if (authStore.loginAuth === 'steemlogin') {
                steemlogin.broadcast(transaction.operations).then((result: any) => {
                ...
                client.broadcast.sendOperations(transaction.operations as [], PrivateKey.fromString(privateKey)).then((result) => {
